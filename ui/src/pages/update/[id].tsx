...
const convertIdToString = (id: string | string[] | undefined): string => {
  if (id == undefined) {
    return "";
  }
  if (Array.isArray(id)) {
...
const UpdatePatient: FunctionComponent = () => {
  ...
  const idStr = convertIdToString(id);
  ...
  const handleUpdate = async (updatedPatient: Patient) => {
    updatePatient(idStr, updatedPatient)
      .then(() => router.push("/"))
      .catch((error) => {
        if (error.response) {
          if (error.response.data) {
